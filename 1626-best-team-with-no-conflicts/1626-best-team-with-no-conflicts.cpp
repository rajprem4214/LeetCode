...
public:
    int bestTeamScore(vector<int>& scores, vector<int>& ages) {
        ...
        //dp[i]:= max score of choosing players[0.....i] w/ player[i] being selected
        vector<int> dp(n);

        for(int i=0 ; i<n ; ++i) players.emplace_back(ages[i] , scores[i]);
        sort(begin(players), end(players) , [](const auto& x , const auto& y){
        ...
        for(int i = 0 ; i<n ; ++i){
            ...
            for(int j = 0 ; j<i ; ++j){
                ...
                if(players[j].score>=players[i].score) dp[i] = max(dp[i] , dp[j]+players[i].score);
            }
